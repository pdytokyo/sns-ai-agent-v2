...
if (process.env.ENABLE_ANALYSIS === 'true') {
  ...
  console.log('Skipping Analysis Features tests because ENABLE_ANALYSIS is not set to true');
  test('Analysis Features - Skipped', () => {
    console.log('Analysis tests are skipped when ENABLE_ANALYSIS is not true');
    test.skip(true, 'Analysis features are disabled');
  });
}
