...
データ駆動型Instagram Reelsスクリプトジェネレーターは、高エンゲージメントコンテンツからターゲットオーディエンスに合わせた台本を生成します：

- **視聴者分析**: コメントからk-meansクラスタリングと正規表現を使用して年齢層・性別・興味関心を抽出
- **フィルタリング**: 指定されたターゲット属性（例：{"age":"18-24","interest":"study"}）に基づいてReelsをフィルタリング
- **エンゲージメント最適化**: 高エンゲージメントコンテンツの構造を分析し、類似の台本構造を生成
- **データ収集**: Playwrightステルスモードを使用してInstagramからデータを収集（Graph API不使用）

## 🗝 クライアントデフォルト設定

各クライアントごとに以下のデフォルト設定を保存・適用できます：

- **デフォルトターゲット**: クライアントの主要ターゲット層（年齢、性別、興味関心）
- **トーン設定**: NGワードや推奨表現などのトーンルール
- **文字数制限**: 台本の最大文字数制限
- **設定の適用**: UIの「保存設定を使う」トグルでデフォルト設定を適用/無視を切り替え
...
- `reels` - Instagram Reelsデータ（メタデータ、トランスクリプト、視聴者情報）
- `client_settings` - クライアント設定（デフォルトターゲット、トーンルール、文字数制限）
